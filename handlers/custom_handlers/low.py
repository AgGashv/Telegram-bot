...
from keyboards.inline.low_continue_ask import ask_to_continue
from loader import bot
from states.flight_info import FlightInfoState
from config_data.config import querystring, direct_url, headers, prices_url
...
def low(message):
    bot.set_state(message.chat.id, FlightInfoState.origin_city, message.chat.id)
    bot.send_message(message.chat.id, "Введите город отправления.")


@bot.message_handler(state=FlightInfoState.origin_city)
def get_origin_city(message):
    try:
        ...
        bot.set_state(message.from_user.id, FlightInfoState.destination_city, message.chat.id)
...
@bot.message_handler(state=FlightInfoState.destination_city)
def get_destination_city(message):
    try:
        ...
        querystring.update({"origin": Cities[id_origin].code,
        ...
        bot.send_message(message.from_user.id, "Хотите продолжить?", reply_markup=ask_to_continue())
